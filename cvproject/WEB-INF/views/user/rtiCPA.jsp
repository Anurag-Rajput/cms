	...
	$.ajax({
		...
		success : function(response) {
			...
			if(response.flag==true){
				...
				window.location = url
				...
				else if(response.accessId==221){
					var anchorValue='window.open("<c:url value="/user/cvccpaOm"/>","nyWindow","width=600,Height=400,top=100,left=400");';
				 	$('#naOm').attr('onclick', anchorValue);
				 	$('#naOm').trigger( "click" );
				}
				
				else if(response.accessId==223){
					alert("in");
					var anchorValue='window.open("<c:url value="/user/cvccpaAcknowledgment"/>","nyWindow","width=600,Height=400,top=100,left=400");';
					...
					var anchorValue='window.open("<c:url value="/user/allNotingDetails/cpa"/>","nyWindow","width=600,Height=400,top=100,left=400");';
  ...
  <div class="main">
    <div class="wrapper">
      <article class="col-1">
         ...
         <div class="inner-text" style="width:250%;  color: #0E0D01;">  &nbsp; <a href="<c:url value="/user/searchCpaComplain"/>">Go Back</a><br/>
            <small> </small> </div>
      </article>
      <article class="center">
      <br></br>  
       <div style="width: 750px; color: navy; background-color: #f3f3f3; border: 2px solid #fff; border-radius: 25px; height: 1540px; 
       padding-left: 10cm; padding: 5px; margin-left: 135px;  border-left-width: 2px; border-top-width: 2px; overflow:auto; " >
       <br></br>
   ...
   <table>
   								...
   								<%-- <tr><td><div class="form-group"  style="padding-left: 3cm;">
										...
										<td><div class="form-list" >
											${complain.complaintSentType}<hr style="color:#00ffe4;">
										</div></td>
								</div></tr> --%>
								...
								<tr><td><div class="form-group"  style="padding-left: 1cm;">
									...
									<td><div class="form-list">
											...
											<hr style="color:#00ffe4;">
										</div></td>
								</div></tr>
								
								<tr><td><div class="form-group"  style="padding-left: 1cm;">
										<label>&nbsp;Bo Decision :</label></td>
										<td><div class="form-list">
...
<p align="center">All Basic Information:&nbsp;&nbsp;<a  id="allDetails"></a>  <a style="color:red;" href="javascript:sessionUpdate('${complain.complaint}','22')"> Click Here </a></p>
	
     
  <c:if test="${complain.boDecision == 'NA'}">
   	<p align="center">Download Office Memorandum:&nbsp;&nbsp;<a id="naOm"></a><a style="color:red;" href="javascript:sessionUpdate('${complain.complaint}','221')"> Click Here </a></p> 
  </c:if>
  
  <p align="center">Download Acknowledgement:&nbsp;&nbsp;<a id="filedAck"></a><a style="color:red;" href="javascript:sessionUpdate('${complain.complaint}','223')"> Click Here </a></p>
	 ...
	 <p align="center">Download Document Details:&nbsp;&nbsp;
			...
			<c:if test="${fn:trim(complain.enc_file_name)!=''}">
	            <c:set var="fileName" value="${complain.file_location}/${complain.enc_file_name}" scope="request" />
	            <a style="color:blue;"  onclick="window.open('<c:url value='/uploadDoc/${fileName}'/>','nyWindow','width=600,Height=400,top=100,left=400');">Click here</a>
            </c:if> 
            <c:if test="${fn:trim(complain.enc_file_name)==''}">
	            <label>No Attachement</label>
            </c:if>  
</p>
